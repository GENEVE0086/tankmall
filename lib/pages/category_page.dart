...
class _LeftCategoryNavState extends State<LeftCategoryNav> {
  ...
  void _getCategory() async {
    await request('getCategory1').then((val) {
...
//      print(data);
      ...
      Provide.value<ChildCategory>(context)
          .getChildCategory(list[0].bxMallSubDto, list[0].categoryId);
...
//          : categoryId,
//      'productId': Provide.value<ChildCategory>(context).productId,
//      'page': 1
//    };
//
...
//    });
    String cateId = categoryId == null
        ? Provide.value<ChildCategory>(context).categoryId
        : categoryId;
    getRequest("http://47.101.180.71:8080/catalog/category/${cateId}/items")
        .then((response){
      var data = json.decode(response.toString());
      CategoryItemListModel itemList = CategoryItemListModel.fromJson(data);
      Provide.value<CategoryItemListProvide>(context)
...
class _RightCategoryNavState extends State<RightCategoryNav> {
  ...
  Widget _rightInkWell(int index, BxMallSubDto item) {
    ...
    return InkWell(
      onTap: () {
        Provide.value<ChildCategory>(context)
            .changeChildIndex(index, item.productId);
        _getGoodList(context, item.productId);
      },
      child: Container(
  ...
  //得到商品列表数据
  void _getGoodList(context, String productId) {
//    var data = {
...
//      }
//    });
    print("http://47.101.180.71:8080/catalog/product/${productId}/items");
    getRequest("http://47.101.180.71:8080/catalog/product/${productId}/items")
        .then((response){
          var data = json.decode(response);
          CategoryItemListModel itemList = CategoryItemListModel.fromJson(data);
          if (itemList.data.itemList == null) {
            Provide.value<CategoryItemListProvide>(context).getItemList([]);
          } else {
            Provide.value<CategoryItemListProvide>(context)
                .getItemList(itemList.data.itemList);
          }
...
class _CategoryGoodsListState extends State<CategoryGoodsList> {
  ...
  @override
  Widget build(BuildContext context) {
    return Provide<CategoryItemListProvide>(
